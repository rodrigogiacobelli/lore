...
|      |  +-- *.yaml            # One file per schema, $id lore://schemas/<kind> (kinds: doctrine-yaml, doctrine-design-frontmatter, knight, watcher, codex, codex-source, artifact, glossary, main-rite, shared-step)
...
|          |- skills/           # Default agent skills, seeded to .lore/skills/ on init (new-doctrine, new-knight, explore-codex, ... and new-custom-schema — scaffolds a .lore/custom-schemas/<kind>.yaml overlay)
|          |  +-- new-custom-schema/SKILL.md   # Scaffolds an add-only custom-frontmatter overlay; enforces the add-only rules and runs lore health to confirm
...
| `config.py` | Project config TOML loader. `@dataclass(frozen=True) Config` with `show_glossary_on_codex_commands: bool = True`, `health_report_retention: str = "none"`, and `extras: Mapping[str, object]` (preserves unknown keys, forward-compatible). `DEFAULT_CONFIG = Config()`. `load_config(root)` reads `.lore/config.toml` via stdlib `tomllib`; returns `DEFAULT_CONFIG` on missing file (silent) or malformed TOML (one-time stderr warning), and drops a single key to its default on a type or value rejection while the rest of the file parses. Three parallel tables drive that: `_FROM_TOML` maps kebab-case TOML keys to snake_case dataclass fields, `_EXPECTED_TYPE` names the accepted Python type per key (`invalid type for <key> ... (expected <type>)`), and `_ALLOWED_VALUES` lists the accepted tokens for constrained string keys (`invalid value for <key> ... (expected one of: ...)`), in the order the warning prints. Adding a setting means one field plus one entry per applicable table. `_warn_once` holds a module-level latch: at most one config warning per process, whatever the kind. `Config` is internal — NOT in `lore.models.__all__` until Realm asks (FR-14, ADR-010) — but IS re-exported through `lore.api.__all__`. Imports stdlib only (`tomllib`, `dataclasses`) plus `lore.paths`. |
