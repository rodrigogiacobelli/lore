...
| `paths.py` | Canonical path helpers for the `.lore/` directory tree. Centralises the magic string `".lore"` and all sub-path arithmetic. Also provides `derive_group(filepath, base_dir)` — derives a GROUP string from intermediate directory components joined with `-`. Provides `watchers_dir(root)`, `glossary_path(root)` → `<root>/.lore/codex/glossary.yaml`, `config_path(root)` → `<root>/.lore/config.toml`, and the custom-schema overlay helpers `custom_schemas_dir(root)` → `<root>/.lore/custom-schemas/` and `custom_schema_path(root, kind)` → `<root>/.lore/custom-schemas/<kind>.yaml` (same shape as `glossary_path`/`config_path`). Imported by `cli.py`, `oracle.py`, `db.py`, `knight.py`, `doctrine.py`, `artifact.py`, `watcher.py`, `glossary.py`, `config.py`, and `schemas/`. |
