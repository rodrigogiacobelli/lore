...
4. **Mission creation.** The orchestrator creates Missions (lore codex show conceptual-entities-mission) within the Quest, pointing each at a doctrine mission with `-D <doctrine>/<mission>` and setting priorities. Missions carry the full description of what the worker needs to do.
...
6. **Claim and dispatch.** The orchestrator claims Missions and dispatches worker agents. Each worker receives a Mission ID and fetches its full context — Mission description and doctrine mission instructions — in a single call.
...
2. **Review the mission** — `lore show <mission-id>` shows the description, the doctrine mission instructions, dependencies, and any board messages from previous agents. Confirm the mission is self-contained before claiming.
