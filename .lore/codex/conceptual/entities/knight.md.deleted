...
This applies to both orchestrator and worker agents. The agent instruction text `lore init` renders — into `.lore/LORE-AGENT.md` and into each selected agent's instruction file — includes explicit guidance for both roles on using artifact commands.
