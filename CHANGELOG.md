...
#### The Knight entity is removed; a doctrine is a directory of prose

A doctrine no longer carries a machine-readable step graph. It is a directory: `<stem>/<stem>.design.md` holding the design prose, plus `<stem>/missions/<id>.md`, one file per reusable instruction. The Knight entity, which held the second copy of that instruction, is removed outright — no shim, no deprecation window, no alias. A Lore mission reaches its instructions through `missions.doctrine_mission`, a `<doctrine>/<mission>` reference resolved at read time. Lore has six entity types.

- **BREAKING: removed from `lore.api.__all__`.** Eight names: the five knight callables `list_knights`, `read_knight`, `create_knight`, `update_knight` and `delete_knight`, and the three dataclasses `Knight`, `Doctrine` and `DoctrineStep`. There is no replacement for the knight callables — a doctrine mission file is the one home for a reusable instruction, reached with `read_doctrine(root, id, mission="<id>")`. `Doctrine` and `DoctrineStep` described a step graph that no longer exists; `DoctrineListEntry` is unchanged. The private CLI-only aliases `lore.api._knight` and `lore.api._validate_frontmatter` are gone with them.
- **BREAKING: removed from `lore.models.__all__`.** `Knight`, `Doctrine` and `DoctrineStep`, whose dataclasses are deleted from the module.
- **BREAKING: two exported callables changed their positional parameters.** `create_doctrine(project_root, name, design_content, missions, *, group=None)` takes the design document's text and a `{mission id: content}` mapping where it took two source paths. `update_doctrine(project_root, name, design_content=None, missions=None, remove_missions=None)` takes the same content, merges missions by id, and soft-deletes the ids in `remove_missions`.
- **BREAKING: four exported callables renamed keyword parameters.** `create_mission`, `update_mission` and `update_mission_full` take `doctrine_mission=` where they took `knight=`, and the two update functions take `remove_doctrine_mission=` where they took `remove_knight=`. `get_mission_detail` takes `include_doctrine_mission=` where it took `include_knight=`.
- **BREAKING: one exported dataclass field renamed.** `Mission.knight` → `Mission.doctrine_mission`.
- **BREAKING: nine exported callables changed their return shape.** `read_doctrine` returns `{id, title, summary, design, missions, origin}` — `raw_yaml` and `steps` are gone, `missions` is the mission index, and a `mission` key carrying `{id, title, summary, body}` or `None` appears when the new `mission=` keyword is passed. `create_doctrine` returns `{created, group, missions, path}` (was `{id, filename, group, design_filename}`). `update_doctrine` returns `{updated, design_replaced, missions_replaced, missions_removed}` (was `{id, filename, updated_at}`). Six mission-bearing envelopes carry `doctrine_mission` where they carried `knight` — `get_mission_detail` (also `doctrine_mission_contents` for `knight_contents`), `list_missions`, `list_missions_grouped`, `read_mission`, `get_quest_detail` and `get_ready_missions`. `list_doctrines` and `delete_doctrine` are unchanged.
- **BREAKING: the `lore knight` command group is removed.** `lore knight list|show|new|edit|delete` are unknown commands at exit 2. `lore health --scope knights` is a `click.Choice` rejection at exit 2; the scope vocabulary is ten tokens. `.lore/knights/` is no longer written, read, seeded or un-ignored, and `lore init` removes the seeded `.lore/knights/default/` tree one last time, naming every file it removes and every knight a project authored outside it.
- **BREAKING: CLI flags removed and renamed.** `lore new mission` and `lore edit` take `-D/--doctrine-mission` and `--no-doctrine-mission` where they took `-k/--knight` and `--no-knight`; `lore show <mission>` takes `--no-doctrine-mission`, prints `Doctrine Mission:` and a `--- Mission Instructions ---` section, and is silent when a reference does not resolve — `lore health --scope doctrines` is that condition's single reporter. `lore doctrine new` takes `-d` and a space-separated `-m`; its `-f/--from` is gone. `lore doctrine edit` takes `-d`, `-m` and `--remove-mission`; its `-f/--from` and its read-from-stdin fallback are gone, and a run naming none of the three is a usage error at exit 2. `lore doctrine show` gains `--mission <id>`, and both its JSON envelopes are now wrapped — `{"doctrine": …}` and `{"mission": …}`.
- **BREAKING: the `knight` kind is gone from `update_frontmatter_fields`,** which raises `Unknown kind: knight`. The `doctrine` kind now edits `<doctrine>/<stem>.design.md` rather than the deleted `<stem>.yaml`. `lore.paths.knights_dir` is deleted and `entity_location(root, "knight")` raises.
- **BREAKING: schema migration v6 → v7 drops every stored knight value.** `missions.knight` is replaced by `missions.doctrine_mission`, which starts empty on every row, and `mission_type = 'knight'` becomes `'agent'`. A stored value was a bare knight name and is never a valid `<doctrine>/<mission>` reference, so carrying it across would put every upgraded project into `lore health` errors on missions nobody touched. Which knight files a project still holds is reported by `lore init` instead: seeded knights are removed and named, and knights the project authored are left on disk and named as no longer read.
- **Packaged schemas 12 → 11.** `doctrine-mission-frontmatter` is added; `knight-frontmatter` and `doctrine-yaml` are deleted, and `load_schema` raises `FileNotFoundError` for both.
