...
# ---------------------------------------------------------------------------
# The ingest-source and refresh-source structural guards that stood here were
# removed with their subjects: interactive-init-us-005 absorbs both skills into
# `store-memory`, so `src/lore/defaults/skills/ingest-source/` and
# `refresh-source/` no longer ship. The catalogue's `retired` ledger records
# where each went, and tests/unit/test_skills.py asserts that neither directory
# survives.
# ---------------------------------------------------------------------------
