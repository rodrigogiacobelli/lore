...
def test_schemas_is_last_in_all_scopes():
    ...
    Note: bindings (US-001), rites (US-006), voice, and skills were appended
    after schemas, so schemas is no longer at a fixed tail index; the durable
    invariant is that schemas still ships in the default-all scope set and
    precedes the later additions (bindings, then rites, then voice, then
    skills last).
    """
    assert "schemas" in _ALL_SCOPES
    assert _ALL_SCOPES[-1] == "skills"
