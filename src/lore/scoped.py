...
def select(
    ...
    *,
    ...
    a doctrine's directory name, say, which the design document's frontmatter
    ``id`` is free to disagree with. It is consulted only for this project's own rows and only
...
def locate(project_root: Path, scope: str | None, record: dict) -> tuple[Path, str]:
    ...
    For the modules whose listing carries no path — doctrines, watchers and
    rites — this is how a scoped ``read_*`` reaches the file:
