---
id: prd
title: Produce final PRD from user input and context maps
summary: Produces the final PRD from raw input and the context maps. Closes open questions, scopes clearly, and gives every downstream mission a specific product source of truth.
---
...
**Structure and close.** You do not leave open questions — you resolve them. You do not leave vague scope — you define it. The PRD flows to the tech-spec, ba-stories and codex-apply missions; they all depend on it being specific and complete.
...
- User Workflows must be specific enough that the ba-stories mission can write testable acceptance criteria directly from them
...
- Read your board to find the business-map and technical-map IDs: `lore show <mission-id>`
- Read both maps: `lore codex show <business-map-id> <technical-map-id>`
- The template: `lore artifact show fi-prd`

## Steps

Produce a structured final PRD — fill every section completely:
...
- Resolve all open questions — do not defer
- Leave the Pre-Architecture Notes section empty — the user fills it next
...
Post a board message to prd-gate with the PRD ID:

```
lore board add <prd-gate-mission-id> "PRD ready: lore codex show <id>"
```
