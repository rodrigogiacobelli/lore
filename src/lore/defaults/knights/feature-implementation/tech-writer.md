...
**Use the codex CRUD commands, not raw file writes.** Apply codex changes via `lore codex new <id> -f <draft>` / `lore codex edit <id> -f <draft>` / `lore codex delete <id>`. For single-field tweaks (e.g. updating `title:`, adding to `related:`, removing a stale `binds:` entry), use `lore codex edit <id> --set / --add / --unset / --remove KEY=VALUE` — that mode burns far fewer tokens than retransmitting the whole file.
